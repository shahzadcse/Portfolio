...
/********** additional script by Shahzad Hussain *******/
// this is the code which runs when page is loaded.
window.onload = function () {
    //calling sorting function  
    data.sort();
    var flag = 1; // flag for initial loading of data
    var i = 0;
    // create the UL element that our LI elements will descend from
    eUL = document.getElementById("menus");
    // loop over the length of the "data" array
    for (i = 1; i <= data.length; i++) {
        // create an LI
        eLI = document.createElement("li");
        // append the value of data[i] to the LI as a text node
        var loadObj = data[i - 1];
        var fname = loadObj['firstName'];
        var lname = loadObj['lastName'];
        var pic = loadObj['picture'];
        var bio = loadObj['bio'];
        eLI.appendChild(document.createTextNode(data[i]));
        var elementHtml = "<a href='#' id='" + i + "' onclick='javascript:fetchData(this);'><span>" + fname + "</span> <span>" + lname + "</span></a>" + " <div class ='article' style='display:none' ><img id='picture" + i + "' alt='profile picture'/> <h2><span id='firstName" + i + "'></span><span id='lastName" + i + "'></span></h2><span id='bio" + i + "'></span></div>";
        eLI.innerHTML = elementHtml;

        // append the LI to the UL
        eUL.appendChild(eLI);

        // loading data only once when page loads 
        if (flag == 1) {
            flag = 0;
            document.getElementById("firstName1").innerHTML = fname;
            document.getElementById("lastName1").innerHTML = lname;
            document.getElementById("picture1").src = pic;
            document.getElementById("bio1").innerHTML = bio;
        }
    }
    //making first element selected
    document.getElementById('1').className = 'active';
    // show data block on page load 
    next(document.getElementById('1')).style.display = 'block';
}


/**** sorting all elements of the 'data' array alphabetically. *******/
...
/******* Function shows menus selected or deselected. *******/
function selected(obj) {
    ...
    next(obj).style.display = "block";

}

/******* function is used to retrive data from 'data' object.*****/

function fetchData(thisobj) {

    selected(thisobj); // make it selected tab.
    var index = parseInt(thisobj.id) - 1; //as index starts from zero 
    var innerObject = data[index]; //accessing member of 'data' object
    index++;
    var fname = innerObject['firstName']; // get first name value
    var lname = innerObject['lastName']; // get second name value

    document.getElementById("firstName" + index).innerHTML = fname;
    document.getElementById("lastName" + index).innerHTML = lname;
    document.getElementById("picture" + index).src = innerObject['picture']; // get the picture of items
    document.getElementById("bio" + index).innerHTML = innerObject['bio']; // get the picture of bio
